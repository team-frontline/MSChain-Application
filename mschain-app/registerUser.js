...
async function main() {
    try {
        ...
        const secret = await ca.register({ affiliation: 'org2.department1', enrollmentID: 'user2.1', role: 'client' }, adminIdentity);
        const enrollment = await ca.enroll({ enrollmentID: 'user2.1', enrollmentSecret: secret });
        const userIdentity = X509WalletMixin.createIdentity('Org2MSP', enrollment.certificate, enrollment.key.toBytes());
        await wallet.import('user2.1', userIdentity);
